namespace VideoConverter.Options
{
	...
	public class AddCriteriaOption : CommandSettings
	{
		...
		[Description(
			"The episode number to map to. " +
			"[bold]Will not be taken into if a season is not specified, and there is no season in the file name[/]"
		)]
		...
		public override ValidationResult Validate()
		{
			if (string.IsNullOrEmpty(SeriesName) && SeasonNumber is null && EpisodeNumber is null)
			{
				return ValidationResult.Error(
					"You must specify either a series name or a season number (optionally also a episode number)"
				);
			}
